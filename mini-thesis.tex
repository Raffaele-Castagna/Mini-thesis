...
Nodes may have a relationship between eachother:
\begin{itemize}
...
Another important statistical tool for AI Security is Benford's Law (a.k.a. The Law of First Digits) IT states that in many naturally occurring datasets, the leading digit $d$ is not uniformly distributed but follows a logarithmic probability:
...
Where $\Delta f$ is the sensitivity of the function, which is the maximum amount that $f(D)$ can change by adding or removing a person from the data set.
...
\end{itemize}
But there is a trade off, when we increase the sensitivity (\textit{Recall}), it lowers precision, so the detection systems needs to be fine tuned via tresholds (e.g., the $\tau$ in Mahalanobis distance or the $k\sigma$ in ARIMA)\\
